...
- [source](../../../../backend/apps/anomalies/models.py)
...
|------|------|------------|
| `AnomalyEvent` | Class | Reflected directly from the current top-level implementation surface. |
| `AnomalyNote` | Class | Reflected directly from the current top-level implementation surface. |
...
This module sits at `backend/apps/anomalies/models.py` and acts as a concrete implementation boundary inside the repository. Persistence boundary that defines the database entities and relationships used by the surrounding feature.
...
From a surface perspective, the top-level implementation currently exposes or declares `AnomalyEvent`, `AnomalyNote`. That reflected surface is intentionally tied to the source file itself, so if the code changes the document should be regenerated with it.
