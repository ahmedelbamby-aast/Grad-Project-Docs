...
| `GAZE_HORIZONTAL_HEAD_VARIANT` | `coco80`; `gaze2` only for Cycle 9b benchmark candidates | `backend/config/settings/base.py`, `backend/apps/video_analysis/tasks.py`, `tools/prod/prod_start_triton.sh`, `tools/prod/prod_enable_gaze_horizontal_gaze2.sh` | Selects the horizontal-gaze output contract. `coco80` expects `[84,2100]`; `gaze2` expects `[6,2100]` from `gaze_horizontal_gaze2_model`. | `coco80` or `gaze2`. `gaze2` requires matching model route overrides and Triton configs. | `gaze2` trims dense output bytes for horizontal gaze while remapping compact class IDs back to legacy DB IDs `4/5`; rollback is `coco80`. |
