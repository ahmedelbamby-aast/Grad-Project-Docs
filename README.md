...
| `TRITON_BEHAVIOR_TOP_K_ENABLED` | `0` | `backend/config/settings/base.py`, `backend/scripts/build_tensorrt_engines.py`, `tools/prod/prod_enable_behavior_topk.sh` | Enables the Cycle 9b B.2.a candidate route that packs only the top task-valid behavior anchors inside Triton before returning outputs to Python. | Boolean-like values. Requires `GAZE_HORIZONTAL_HEAD_VARIANT=slice`, `behavior_ensemble_gaze_slice_topk`, and four Top-K adapter plans loaded in Triton. | Candidate only until production parity and benchmark pass. Rollback is `0` plus `MODEL_ROUTE_BEHAVIOR_ALL_MODEL_NAME=behavior_ensemble_gaze_slice`. |
