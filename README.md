...
| `EMBEDDING_PREFETCH_TRACK_LOOKUP` | `0` | `backend/config/settings/base.py`, `backend/apps/video_analysis/tasks.py`, `tools/prod/prod_run_cycle13_track_lookup_benchmark.sh` | Enables the Cycle 13.B candidate that resolves embedding `StudentTrack` values from prefetched bounding boxes instead of per-detection ORM lookups. | Boolean-like values. Candidate benchmark wrapper sets `1`; rollback is `0` plus Celery worker restart. | Targets the `66.223 s` track-lookup bucket measured in replay `cycle13-embedding-profile-20260603T003853Z`; it is not accepted until the full production benchmark and correctness comparison pass. |
