...
| **Cycle 12.B** | Bounded behavior-wait overlap dispatcher | **STAGED; PRODUCTION BENCHMARK REQUIRED** | Candidate doc exists and code is guarded by `TRITON_CROP_FRAME_BEHAVIOR_OVERLAP=1`. It starts crop-frame `behavior_all` for batch N before preparing batch N+1, then finalizes batches in order. Missing: deploy, full `combined.mp4` production benchmark, DB/model-agreement parity, RTT/GPU/memory evidence, decision table, rollback proof. | `docs/cycle_12_overlap_dispatcher_investigation.md`, `docs/cycle_12_persistent_dispatcher_results.md`, `tools/prod/prod_run_behavior_overlap_benchmark.sh` |
