...
| **Cycle 12.A** | Persistent async dispatcher measurement | **ACTIVE — PHASE A INSTRUMENTATION STAGED; PROD BENCHMARK PENDING** | B.1 repeat benchmarks proved decode/NMS is small while infer wait plus server/orchestration remains dominant. Staged: `TRITON_ASYNC_DISPATCH_PROFILING`, production metric collection, watcher output, and `tools/prod/prod_run_async_dispatch_profile_benchmark.sh`. Missing: a `combined.mp4` Linux RTX 5090 benchmark that quantifies `async_runner.run(...)` boundary wall before any dispatcher implementation. | `docs/cycle_12_persistent_dispatcher_investigation.md`, `docs/inference_parallelization_plan.md`, `docs/crop_frame_optimization_execution.md` |

### Z.3 Cycles planned but not yet started (restaged after Cycle 12.A)
