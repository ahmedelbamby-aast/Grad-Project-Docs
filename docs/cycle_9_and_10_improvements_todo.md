...
| **Cycle 20** | Streaming DB persistence and embedding overlap with inference | **20.D NOT ACCEPTED / 20.E STAGED_LOCAL 2026-06-06** | r3 restored row parity and model agreement but regressed DB FPS, Step 2 wall, GPU average utilization, and rewrote `4449/4541` packets in Step 3. Cycle 20.E now stages `OFFLINE_STREAM_POST_STAGE_MODE=final_stable_overlap` to remove callback DB writes and persist only final-tracking-stable packets; keep default production `OFFLINE_STREAM_POST_STAGES=0` until a fresh benchmark. | `docs/cycle_20_streaming_persistence_embedding_overlap_investigation.md` |
