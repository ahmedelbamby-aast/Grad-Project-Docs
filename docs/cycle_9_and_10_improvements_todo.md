...
| 6 | **B.3 child critical-path analysis — Step 1 REMEASURED 2026-06-02** | 9b | `gaze_horizontal_model` still dominant but gap shrank to +15 %; per-child ceiling ≤ 4 % Step 2 wall. Cycle 11.A input `320 → 256` was tried and produced a parity warning, but the real benchmark matrix is now required before accepting, rejecting, skipping, or neglecting it. |
...
| **Cycle 11.A** | Behavior input 320 → 256 | **BENCHMARK REQUIRED / UNDECIDED 2026-06-02** | Production built the 256 engines and captured candidate outputs. The synthetic parity probe warned of correctness risk (`posture_model` class agreement `0.695`, `gaze_vertical_model` `0.955`, large centroid drift on three models), but no full `combined.mp4` benchmark has run yet. Tooling now exists: `tools/prod/prod_run_behavior_input_size_matrix.sh` and `tools/prod/prod_collect_benchmark_metrics.py`. Production is currently restored to `TRITON_CROP_BEHAVIOR_INPUT_SIZE=320`, exact slice + Top-K, and all active models are READY. Missing: real 320-vs-256 production benchmark metrics. | `docs/cycle_11_input_size_investigation.md`, `docs/cycle_11_input_size_results.md` |
...
| **Cycle 11.B / B.3 Step 2** | Kernel-tactic or batch-profile tuning on the dominant child at 320 | **PLANNED, but do not use it to skip 11.A benchmarking** | bounded at ~4 % Step 2 wall | `docs/cycle_9b_child_critical_path_remeasure_topk_results.md`, `docs/cycle_11_input_size_results.md` |
