...
| **Cycle 11.A** | Behavior input 320 → 256 | **NOT ACCEPTED AT PARITY GATE 2026-06-02** | Production built the 256 engines and captured candidate outputs, but the required pre-benchmark parity probe failed (`posture_model` class agreement `0.695`, `gaze_vertical_model` `0.955`, large centroid drift on three models). No full `combined.mp4` benchmark was run. Production was rolled back to `TRITON_CROP_BEHAVIOR_INPUT_SIZE=320`, exact slice + Top-K, and all active models are READY. Missing: either a real-crop parity harness before retrying smaller input, or implementation of lower-risk 11.B/B.3 kernel-tactic tuning. | `docs/cycle_11_input_size_investigation.md`, `docs/cycle_11_input_size_results.md` |
...
| **Cycle 11.B / B.3 Step 2** | Kernel-tactic or batch-profile tuning on the dominant child at 320 | **PLANNED AFTER 11.A REJECTION** | bounded at ~4 % Step 2 wall | `docs/cycle_9b_child_critical_path_remeasure_topk_results.md`, `docs/cycle_11_input_size_results.md` |
