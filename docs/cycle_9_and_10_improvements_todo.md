...
| **Cycle 12.C** | Single-inflight behavior overlap | **NEXT TASK — INVESTIGATION REQUIRED BEFORE CODE** | Keep the useful 12.B overlap but avoid two behavior jobs in flight: build current crop payloads, finalize pending behavior, then submit current behavior. Missing: investigation doc, implementation, production benchmark, decision table. | planned: `docs/cycle_12_single_inflight_overlap_investigation.md` |
