...
- [source](../../../../frontend/src/stores/visibilityStore.ts)
...
|------|------|------------|
| `useVisibilityStore` | Export | Reflected directly from the current top-level implementation surface. |
...
This module sits at `frontend/src/stores/visibilityStore.ts` and acts as a concrete implementation boundary inside the repository. Client state container that centralizes domain data and update logic for the frontend runtime.
...
From a surface perspective, the top-level implementation currently exposes or declares `useVisibilityStore`. That reflected surface is intentionally tied to the source file itself, so if the code changes the document should be regenerated with it.
