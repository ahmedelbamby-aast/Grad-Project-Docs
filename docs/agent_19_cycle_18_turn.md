...
| CI visibility | `.gitignore` | Explicit exceptions keep the six CI-read JSON artifacts visible. |
...
| Deep root-cause investigation | `docs/cycle_18_identity_association_root_cause_investigation.md` | Records primary-source best practices, the two confirmed root causes, applied governance, measurements, and final decision. |
| Label-invariant tracking probe | `tools/prod/prod_probe_cycle18_label_invariant_tracking.py` | Read-only one-to-one association evaluator for independent local-label namespaces. |
| Label-invariant tracking tests | `backend/tests/unit/pipeline/test_prod_probe_cycle18_label_invariant_tracking.py` | Proves namespace recovery, residual association loss, and no runtime decision authority. |
| Label-invariant production summary | `docs/architecture/cycle18_label_invariant_tracking_probe.production_summary.json` | Records the measured production split, remote evidence hashes, and no-selection result. |
...
| `A19-19` Gate resolvability of Agent 19 contract artifacts | `COMPLETED` | Current one-command gate resolves all `28` inventory entries |
| `A19-20` Gate CI visibility of all Cycle 18 JSON artifacts | `COMPLETED` | Six explicit `.gitignore` exceptions |
...
| `A19-22` Verify repo-relative file and line references in Agent 19-owned docs | `COMPLETED` | Current bounded checker passes with `157` references and `25` line citations |
...
| `A19-44` Deep-audit the evaluation contract and runtime identity blockers | `COMPLETED` | `docs/cycle_18_identity_association_root_cause_investigation.md` |
| `A19-45` Review primary-source tracking evaluation and association practices | `COMPLETED` | Primary-source table and adopted contract in the deep investigation |
| `A19-46` Implement a read-only label-invariant production evaluator | `COMPLETED` | `tools/prod/prod_probe_cycle18_label_invariant_tracking.py` |
| `A19-47` Add focused namespace, fragmentation, and no-selection tests | `COMPLETED` | `backend/tests/unit/pipeline/test_prod_probe_cycle18_label_invariant_tracking.py` |
| `A19-48` Run the production label-invariant probe against completed jobs | `COMPLETED` | Hashed evidence under `/home/bamby/grad_project/backend/logs/cycle18-label-invariant-probe-20260604T163806Z` on production |
| `A19-49` Propagate best practices into constitution and execution guidance | `COMPLETED` | Constitution v2.9.0, AGENTS binding section, and Spec Kit templates |
| `A19-50` Integrate new evidence into the one-command Cycle 18 contract gate | `COMPLETED` | `28` artifacts, `6` visible JSON files, and root-cause metric checks |
| `A19-51` Log all measurements and the final governed decision | `COMPLETED` | Cycle 18 docs, ledger, AGENTS bounded turn record, and tracked summary |
| `A19-52` Re-run focused tests and documentation gates | `COMPLETED` | Validation ledger below |
| `A19-53` Resume Cycle 18 blocker resolution after Agent 20 packet evidence | `COMPLETED` | Confirmed remaining blockers are missing appearance references and missing packet-consuming association |
| `A19-54` Stage default-off boundary appearance references | `COMPLETED` | Preview-frame path carrier plus digest-backed per-track feature references |
| `A19-55` Stage parent-side `appearance_packet` association consumer | `COMPLETED` | Fail-closed geometry-plus-appearance candidate decisions in packet metadata |
...
| `docs/cycle_18_identity_association_root_cause_investigation.md` | Deep blocker/root-cause analysis, research, measurements, step log, and final decision |
| `tools/prod/prod_probe_cycle18_label_invariant_tracking.py` | Read-only independent-run association evaluator |
| `backend/tests/unit/pipeline/test_prod_probe_cycle18_label_invariant_tracking.py` | Focused evaluator tests |
| `docs/architecture/cycle18_label_invariant_tracking_probe.production_summary.json` | Tracked production metrics and evidence hashes |
| `backend/apps/pipeline/multi_model.py` | Preview-frame path carrier for default-off Cycle 18 appearance packets |
| `backend/apps/video_analysis/tasks.py` | Pass preview-frame evidence to boundary packet construction only under existing offline sharding flow |
| `backend/apps/video_analysis/services/offline_sharding.py` | Default-off `appearance_packet` producer/consumer candidate |
| `backend/config/settings/base.py` | Default-off, bounded Cycle 18 appearance packet flags |
...
| Implement a Redis boundary cache | Cycle 18 Redis runtime gate |
...
| Cycle 18/19 focused tests | Passed: `39` tests across validator, projector, recovery auditor, structural drift, doc-reference, measured-value, both identity probes, Cycle 19 eligibility, and contract gates |
...
| Contract artifact inventory | Passed: all `28` Agent 19 contract docs, JSON artifacts, helpers, focused tests, and `.gitignore` resolve |
| CI file visibility | Passed: all six contract/projection/evidence JSON files have explicit visibility exceptions and appear in `git status` |
...
| Agent 19 reference resolvability | Passed: all references across four bounded sources and all `25` line citations resolve |
...
| Cycle 18 root-cause result | Both `RAW_LABEL_CONTRACT_DEFECT_CONFIRMED` and `RESIDUAL_ASSOCIATION_GAP_CONFIRMED`; runtime candidate remains unselected |
...
state. The reference verifier checks all three owned docs plus only Agent 19's
...
The final deep root-cause continuation corrected the evaluation contract
without weakening the runtime gate. A read-only production probe measured
shard-1 minimum raw-label F1 at `2.917 %` and label-invariant
global-assignment F1 at `79.876 %`, proving that raw local-ID equality was an
invalid cross-run identity gate. Geometry remained `100.000 %`, while the
largest shard-1 residual association gap remained `20.124 pp`; fragmented and
