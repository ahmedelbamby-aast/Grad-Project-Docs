...
| 2 | Production TensorRT FP32 engine is built with TRT `10.16.1.11`, installed as `osnet_ain_x1_0/1/model.plan`, and loaded `READY` on offline Triton. | `PRODUCTION_BUILT_READY`: build tag `osnet-ain-reid-20260605T20260605T201255Z`; engine digest `93c7b1a6562ff21d51f46129259545aa144fbdaa275ed42b73df1021c6c1cfff`; Triton repository index returned `osnet_ain_x1_0` state `READY`. |
| 3 | PyTorch-vs-Triton parity probe records cosine parity, latency, determinism, digests, JSON, and Markdown. | `PRODUCTION_PARITY_PASSED`: `backend/logs/osnet-ain-reid-parity-20260605T201608Z/reid_triton_parity.json` reports `overall_passed=true`, minimum cosine `0.9999725818634033`, repeat cosine `1.0`, mean Triton latency `19.329396076500416 ms`. |
...
| Ready model list | Existing offline models returned `READY` through `/v2/repository/index`; after the production build, `osnet_ain_x1_0` also returned `READY`. |
...
| 13 | 2026-06-05 | First production OSNet build attempt failed before export. | Replay tag `osnet-ain-reid-20260605T200108Z` stopped during pinned Torchreid install because pip build isolation lacked `numpy`; wrapper updated to use `--no-build-isolation --no-deps` so the production venv supplies installed dependencies. No engine was built and no benchmark lock was held. |
| 14 | 2026-06-05 | Second production OSNet build attempt also failed before export. | Replay tag `osnet-ain-reid-20260605T200213Z` stopped during Torchreid metadata generation because setup required `Cython`; wrapper changed from pip installation to a pinned source checkout under `backend/models/source_deps/deep-person-reid` and exports `PYTHONPATH` before running the builder. No engine was built and no benchmark lock was held. |
| 15 | 2026-06-05 | Narrowed OSNet export/probe imports to the pinned architecture file. | `build_tensorrt_engines.py` and `prod_probe_reid_triton.py` now load `torchreid/models/osnet_ain.py` directly from `TORCHREID_SRC_DIR` / `PYTHONPATH`, avoiding Torchreid package setup and optional Cython imports. Local focused ReID, shard-merge, live-isolation, compile, and shell syntax checks passed. |
