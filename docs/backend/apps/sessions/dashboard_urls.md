# backend/apps/sessions/dashboard_urls.py

## Related Documents

- [source](../../../../backend/apps/sessions/dashboard_urls.py)
...
flowchart TB
    subgraph Source["Current Module"]
        SRC["backend/apps/sessions/dashboard_urls.py\nDashboard endpoints — mounted at /api/v1/dashboard/."]
    end
    subgraph Dependencies["Project Dependencies"]
        I1["apps.anomalies.views\nProject Module"]
        I2["apps.audit.views\nProject Module"]
        I3["apps.sessions.views\nProject Module"]
    end
...
This module sits at `backend/apps/sessions/dashboard_urls.py` and acts as a concrete implementation boundary inside the repository. Routing boundary that exposes concrete URL patterns for the surrounding application surface.

From a dependency perspective, the file currently reaches into `apps.anomalies.views`, `apps.audit.views`, `apps.sessions.views`. Those links were read from the real source file so the diagram reflects the actual local coupling rather than an inferred architecture.
