# backend/apps/sessions/consumers.py

## Related Documents

- [source](../../../../backend/apps/sessions/consumers.py)
...
|------|------|------------|
| `SessionCommentsConsumer` | Class | Reflected directly from the current top-level implementation surface. |
...
This module sits at `backend/apps/sessions/consumers.py` and acts as a concrete implementation boundary inside the repository. WebSocket interaction boundary that receives live events and emits real-time updates to subscribed clients.
...
From a surface perspective, the top-level implementation currently exposes or declares `SessionCommentsConsumer`. That reflected surface is intentionally tied to the source file itself, so if the code changes the document should be regenerated with it.
