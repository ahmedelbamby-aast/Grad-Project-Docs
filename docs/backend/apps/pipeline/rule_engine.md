...
- [source](../../../../backend/apps/pipeline/rule_engine.py)
...
|------|------|------------|
| `RuleEngine` | Class | Reflected directly from the current top-level implementation surface. |
...
This module sits at `backend/apps/pipeline/rule_engine.py` and acts as a concrete implementation boundary inside the repository. Python implementation module that contributes behavior to the repository runtime.
...
From a surface perspective, the top-level implementation currently exposes or declares `RuleEngine`. That reflected surface is intentionally tied to the source file itself, so if the code changes the document should be regenerated with it.
