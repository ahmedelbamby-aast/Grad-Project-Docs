# backend/apps/pipeline/layers/__init__.py

## Related Documents

- [source](../../../../../backend/apps/pipeline/layers/__init__.py)
...
flowchart TB
    subgraph Source["Current Module"]
        SRC["backend/apps/pipeline/layers/__init__.py\nThis document reflects the backend feature module exactly as it is implemented i"]
    end
    subgraph Dependencies["Project Dependencies"]
        I1["apps.pipeline.layers.depth_gaze\nProject Module"]
        I2["apps.pipeline.layers.horizontal_gaze\nProject Module"]
        I3["apps.pipeline.layers.posture\nProject Module"]
        I4["apps.pipeline.layers.vertical_gaze\nProject Module"]
    end
...
This module sits at `backend/apps/pipeline/layers/__init__.py` and acts as a concrete implementation boundary inside the repository. Python implementation module that contributes behavior to the repository runtime.

From a dependency perspective, the file currently reaches into `apps.pipeline.layers.depth_gaze`, `apps.pipeline.layers.horizontal_gaze`, `apps.pipeline.layers.posture`, `apps.pipeline.layers.vertical_gaze`. Those links were read from the real source file so the diagram reflects the actual local coupling rather than an inferred architecture.
