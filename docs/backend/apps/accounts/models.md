...
- [source](../../../../backend/apps/accounts/models.py)
...
|------|------|------------|
| `Role` | Class | Reflected directly from the current top-level implementation surface. |
| `User` | Class | Reflected directly from the current top-level implementation surface. |
...
This module sits at `backend/apps/accounts/models.py` and acts as a concrete implementation boundary inside the repository. Persistence boundary that defines the database entities and relationships used by the surrounding feature.
...
From a surface perspective, the top-level implementation currently exposes or declares `Role`, `User`. That reflected surface is intentionally tied to the source file itself, so if the code changes the document should be regenerated with it.
