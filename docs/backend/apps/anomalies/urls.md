# backend/apps/anomalies/urls.py

## Related Documents

- [source](../../../../backend/apps/anomalies/urls.py)
...
This module sits at `backend/apps/anomalies/urls.py` and acts as a concrete implementation boundary inside the repository. Routing boundary that exposes concrete URL patterns for the surrounding application surface.

From a dependency perspective, the file currently reaches into `apps.anomalies.views`, `core.routers`. Those links were read from the real source file so the diagram reflects the actual local coupling rather than an inferred architecture.
