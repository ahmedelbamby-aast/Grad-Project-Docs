...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS** (19 landed: every Django app + `frontend.src.api` + `frontend.src.hooks`; remaining frontend: `components`, `features/bsil`) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
