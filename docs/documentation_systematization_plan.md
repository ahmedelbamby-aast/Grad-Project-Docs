...
| **2** | System-level entity docs (top-level systems) | **IN PROGRESS** (2 of ~6 landed: `offline_inference_pipeline`, `live_streaming_pipeline`) | One doc per system in `docs/entity/systems/` following the per-entity template, each carrying position + call-graph + external-connections diagrams using the Cycle 0 theme contract. Per § 19.7 one entity per commit |
