...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS** (18 landed: every Django app + first frontend package `frontend.src.api`; remaining frontend: `hooks`, `components`, `features/bsil`) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
