...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS** (20 landed: every Django app + `frontend.src.api` + `frontend.src.hooks` + `frontend.src.components`; remaining frontend: `features/bsil` only) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
