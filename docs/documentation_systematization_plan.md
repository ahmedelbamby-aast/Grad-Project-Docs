...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS — backend apps closed** (17 of ~18 landed: every Django app from `apps.video_analysis` through `apps.telemetry_mcp` now has a module entity doc; remaining is the frontend packages — see § 5 acceptance list) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
