...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS** (4 of ~18 landed: `apps.video_analysis`, `apps.pipeline`, `apps.tracking`, `apps.telemetry`) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
