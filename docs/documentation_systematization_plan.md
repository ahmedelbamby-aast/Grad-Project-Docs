...
| **3** | Module-level entity docs (Django apps + frontend packages) | **IN PROGRESS** (16 of ~18 landed: `apps.video_analysis`, `apps.pipeline`, `apps.tracking`, `apps.telemetry`, `apps.cameras`, `apps.detections`, `apps.anomalies`, `apps.sessions`, `apps.health`, `apps.behavior`, `apps.contracts`, `apps.accounts`, `apps.audit`, `apps.recordings`, `apps.exams`, `apps.exports`) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
