...
|---|---|---|---|
| **0** | Plan + governance hardening | **IN PROGRESS** (this commit) | Three planning docs land + constitution Section 19 ratified + AGENTS.md hardened + README updated |
| 1 | Inventory + reading-order refresh for ALL project markdown | PLANNED | Every project-owned `.md` carries a `**Last updated:**` header; README reading order covers every narrative doc; CI gate passes against the full list |
| 2 | System-level entity docs (top-level systems) | PLANNED | One doc per system in `docs/entity/systems/` following the per-entity template, each carrying system-context + container + deployment diagrams using the Cycle 0 theme contract |
| 3 | Module-level entity docs (Django apps + frontend packages) | PLANNED | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs |
