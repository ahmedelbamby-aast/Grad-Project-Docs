...
| **3** | Module-level entity docs (Django apps + frontend packages) | **CLOSED 2026-06-03** (21 of 21 modules: 17 Django apps + 4 frontend packages — `frontend.src.api`, `frontend.src.hooks`, `frontend.src.components`, `frontend.src.features.bsil`) | One doc per module in `docs/entity/modules/`; covers internal call graph, cross-module deps, DB tables, env vars, public APIs. All 21 commits pre-checked locally with `mmdc` per § 19.3.1. Every Symbol citation grep-verified against the live tree per § 19.6. |
| **4** | Phase / optimization-cycle docs | **IN PROGRESS 2026-06-03** (8 of ~12 cycles landed: `cycles_1_to_5_bundle` `74ec0432`; `cycle_6_pose_chunking` `a1a448b9`; `cycle_7_redis_client_cache` `515fe118`; `cycle_8_embedding_stage` `d2de80a0`; `cycle_9_behavior_ensemble` `c1651663` NOT-ACCEPTED; `cycle_9b_b2b_exact_slice` `7933c1e5`; `cycle_9b_b2c_topk` `be4ba9ee` current accepted baseline; `cycle_10_lpm_phase1` `17075418`+`21666815` NOT-ACCEPTED). Pending: 11 input-size NOT-ACCEPTED, 12 dispatcher variants, 13 embedding-stage, 14a pose-tail, 16b coalescing. | Per accepted/rejected cycle: a single canonical entity doc in `docs/entity/cycles/` that folds investigation + results + status. Each commit pre-checked locally with `mmdc` per § 19.3.1; every File / Symbol / Commit / Job row grep-verified against the live tree per § 19.6. |
